<template>
  <div class="app-container">
    <div class="filter-container">
          <el-input placeholder="请输入评论机器人名称" style="width:200px" class="filter-item"/>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="评论内容" prop="comment_text">
          <el-col :span="24">
            <el-input v-model="temp.comment_text" />
          </el-col> 
        </el-form-item>
...
import { getCommentRobotsList,createCommentRobots,updateCommentRobots } from '@/api/manager'
  ...
  methods:{
    ...
    createData(){
       this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createCommentRobots(tempData).then(() => {
    ...
    updateData(){
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          const tempData = Object.assign({}, this.temp)
          updateCommentRobots(tempData).then(() => {
    ...
    // 获取列表数据
    getList(){
      getCommentRobotsList(this.listQuery).then(response => {
